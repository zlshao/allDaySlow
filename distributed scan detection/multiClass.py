...
filename="202006031400.pcap.Mixed_rseed-2222.pcap.bit_25.thre_80.ratio_32.IP.csv"  #输入文件,测试集
outCsvFile='output/midFile/outMid_'+filename   #中间输出文件
outScannerIPFile1='output/txtFile/outScanner_'+filename+'_distinct.txt'  #输出扫描器IP地址,分类
outScannerIPFile2='output/txtFile/outScanner_'+filename+'_all.txt' #输出扫描器IP地址，不分类

#---------------------------------------------将预测标签写入测试集-----------------------------------------
def writeprelabel(filename,prelabel):
    df = pd.read_csv(filename,index_col=False)
    df.loc[:,'prelabel'] = prelabel
...
def detect():
    ...
    clf = DecisionTreeClassifier(criterion='entropy', max_depth=10, min_samples_leaf=1, random_state=99,
                                 splitter='best')  #
    ...
    print('检测的时间为： %s Seconds'%(end_detect-start_detect))
...
if __name__=='__main__':
    detect()
    start_outputIP=time.perf_counter()
    writeScore(filename)
    # extractScan(outCsvFile)
    extractScanAll(outCsvFile)
    end_outputIP=time.perf_counter()
    print('过滤并输出扫描器IP的时间为： %s Seconds' % (end_outputIP - start_outputIP))
